...
impl Config {
    fn new(args: &[String]) -> Result<Config, String> {
        ...
        if non_flag_args.len() < 2 {
            return Err("Error: No input files proved.".to_string());
        }
...
fn grep_file(file: &str, cfg: &Config) {
    match File::open(file) {
        Ok(f) => {
            let mut reader = BufReader::new(f);
            grep(&mut reader, &cfg.pattern, cfg.case_insensitive);
        }
        Err(e) => {
            eprintln!("Count not open file: {}", e)
        }
    }
}
